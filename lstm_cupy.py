...
def sample(memory, seed_ix, n):
  ...
  """
    ...
    for t in range(n):
        ...
        # compute the forget gate
        # f = sigmoid(Wf * z + bf)
        fs = sigmoid(cp.dot(Wf, zs) + bf)
        # compute the input gate
        # i = sigmoid(Wi * z + bi)
        ins = sigmoid(cp.dot(Wi,zs) + bi)
        # compute the candidate memory
        # c_ = tanh(Wc * z + bc)
        c_s = cp.tanh(cp.dot(Wc,zs) + bc)

        # new memory: applying forget gate on the previous memory
        # and then adding the icput gate on the candidate memory
        # c_t = f * c_(t-1) + i * c_
        cs = fs * c + ins * c_s

        # output gate
        #o = sigmoid(Wo * z + bo)
        os = sigmoid(cp.dot(Wo, zs) + bo)
        hs = os * cp.tanh(cs)
        # DONE LSTM
        # output layer - softmax and cross-entropy loss
        # unnormalized log probabilities for next chars
